...
class CashRegisterUtil extends Util
{
    ...
    public function addSellPayments($transaction, $payments)
    {
        ...
        foreach ($payments as $payment) {
            ...
            if ($payment_amount != 0) {
                ...
                if (isset($payment['cash_amount']) && isset($payment['mpesa_amount']) && $payment['cash_amount'] > 0) {
                    //cash register
                    $cash_payments_formatted[] = new CashRegisterTransaction([
                    ...
                    $mpesa_payments_formatted[] = new CashRegisterTransaction([
                        'amount' => $payment['mpesa_amount'],
                        'pay_method' => 'mpesa',
                        'type' => $type,
                        'transaction_type' => $transaction->type,
                        'transaction_id' => $transaction->id,
        ...
        if (!empty($cash_payments_formatted)) {
            $register->cash_register_transactions()->saveMany($cash_payments_formatted);
        }

        if (!empty($mpesa_payments_formatted)) {
            $register->cash_register_transactions()->saveMany($mpesa_payments_formatted);
        }
