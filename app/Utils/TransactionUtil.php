...
class TransactionUtil extends Util
{
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    if (isset($payment['cash_amount']) && isset($payment['mpesa_amount']) && $payment['cash_amount'] > 0) {
                        //cash register
                        $cash_payment_data = $payment_data;
                        $cash_payment_data['amount'] = $payment['cash_amount'];
                        $cash_payment_data['method'] = 'cash';
                        $cash_payments_formatted[] = new TransactionPayment($cash_payment_data);

                        //mpesa register
                        $mpesa_payment_data = $payment_data;
                        $mpesa_payment_data['amount'] = $payment['mpesa_amount'];
                        $mpesa_payment_data['method'] = 'mpesa';
                        $mpesa_payments_formatted[] = new TransactionPayment($mpesa_payment_data);
                    } else {
        ...
        if (!empty($cash_payments_formatted)) {
            $transaction->payment_lines()->saveMany($cash_payments_formatted);
        ...
        if (!empty($mpesa_payments_formatted)) {
            $transaction->payment_lines()->saveMany($mpesa_payments_formatted);
            $payment_lines = $transaction->payment_lines;
            foreach ($account_transactions as $account_transaction) {
                $payment = $payment_lines->where('payment_ref_no', $account_transaction['payment_ref_no'])->first();
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        \Log::info("TRANSACTION PAYMENT DONE FROM TRANSACTION UTIL");
