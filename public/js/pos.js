$(document).ready(function () {
    ...
    var channel = Echo.channel(`cashier.${user_id}`);

    channel.listen('.stkpush.sent', (data) => {

        let parsedData = typeof data.data === 'string' ? JSON.parse(data.data) : data.data;
        if (parsedData.status == 'success') {

            let MerchantRequestID = data.transaction.MerchantRequestID;
            let CheckoutRequestID = data.transaction.CheckoutRequestID;
            //find if the values exist in the local storage
            let merchant = localStorage.getItem('MerchantRequestID');
            let checkout = localStorage.getItem('CheckoutRequestID');
            ...
            // $('[name="final_total"]').val(total_cart_amount);
            // $('#amount_0').val(total_cart_amount);
            // $('#mpesa_phonenumber').val(c2b_payment_phone);
            // $('#mpesa_cashamount').val(amount_difference);
            // $('#mpesa_amount').val(c2b_payment_amount);
            // $('#recent_payment_modal').modal('hide');
            // pos_form_mpesa.submit();

            toastr.success(parsedData.message);


            $('#pos-save').prop('disabled', false);
            $('#pos-save-mpesa').prop('disabled', false);
        }
    ...
    channel.listen('.mpesa-paymentdata-received', (data) => {

        let parsedData = typeof data.data === 'string' ? JSON.parse(data.data) : data.data;
        if (parsedData.status == 'success') {
            console.log("MPESA DATA RECEIVED", parsedData)
            toastr.success(parsedData.message);

            let full_message = `Payment received. Phone Number: ${parsedData.phone} , Amount: ${parsedData.amount}  and Transaction ID: ${parsedData.receipt_no}`

            toastr.success(full_message);
            $('div#mpesa_details_modal').modal('hide');
            let form_data = localStorage.getItem('formData');
            if (form_data != null) {
                let transaction_data = JSON.parse(form_data);
                if (transaction_data.cash_amount > 0) {
                    // $('.payment_types_dropdown').val('cash');
                    // $('#amount_0').val(transaction_data.cash_amount);
                    // $('[name="final_total"]').val(transaction_data.cash_amount);
                    // $('#mpesa_phonenumber').val(transaction_data.phone_number);
                    // pos_form_obj.submit();
        ...
        if (parsedData.status == 'error') {
            toastr.error(parsedData.message);
            $('#pos-save').prop('disabled', false);
            $('#stkpush-on').val(0);
    ...
    var c2bchannel = Echo.channel(`mpesa.c2b.data`);


    c2bchannel.listen('.mpesa-c2bdata-received', (data) => {

        let parsedData = typeof data.data === 'string' ? JSON.parse(data.data) : data.data;

        // let element = $('#c2b-data-container');
        // element.html('');

        // toastr.success("New M-PESA  Payment!");

    });

    var stkchannel = Echo.channel(`stk-push-channel.${user_id}`);


    stkchannel.listen('.mpesa-stkpushdata-processed', (data) => {

        let parsedData = typeof data.data === 'string' ? JSON.parse(data.data) : data.data;
        var formDataObject = pos_form_obj.serializeArray().reduce(function (obj, item) {
            obj[item.name] = item.value;
            return obj;
        }, {});

        var formData = formDataObject;
        let unprocessed_cart_amount = formData.final_total;
        let total_cart_amount = parseFloat(unprocessed_cart_amount.replace(/[^\d.]/g, ''));
        ...
        if (parsedData.status == 'success') {
            ...
            $('[name="final_total"]').val(total_cart_amount);
            $('#amount_0').val(total_cart_amount);
            $('#mpesa_phonenumber').val(parsedData.phone);
            $('#mpesa_amount').val(parsedData.amount);
            pos_form_mpesa.submit();
            $('div#mpesa_details_modal').modal('hide');
            $('#modal_payment').modal('hide');
            $('#stkpush-on').val(0);
            $('input#search_product').prop('disabled', false);
            markSTKPushPaymentAsUsed(parsedData.receipt_no);
        }
    ...
    $('table#pos_table tbody').on('click', 'i.pos_remove_row', function () {
        ...
        if (stkpush_on == 1) {
            toastr.warning('Please wait for the M-PESA payment to complete');
            return;
        }
        $(this)
            .parents('tr')
    ...
    $('#pos-save').click(function () {
        ...
        if (pay_method == 'mpesa') {
            ...
            var mpesa_method = $('#mpesa_method').val();
    ...
    $('#pos-use-mpesac2b').click(function () {
        ...
        console.log("PAYMENT METHOD:  ", formData.method)
        console.log("M-PESA PAYMENT DATA: ", formDataObject)
        ...
        console.log("FROM DATA: ", formDataObject)
        console.log("Serialized FORM: ", pos_form_obj)

    });


    //hide phone field when the m-pesa method changes.
    $('#mpesa_method').change(function () {
        console.log("mpesa method changed")
        var mpesa_method = $('#mpesa_method').val();
        console.log(mpesa_method)
        mpesa_method == 'mpesa_c2b' ? $('#phone_number_container').hide() : $('#phone_number_container').show()
        mpesa_method == 'mpesa_c2b' ? $('#pos-save-mpesa').html('Confirm Payment') : $('#pos-save-mpesa').html('Finalize Payment')
    })
    ...
    function initializeStkPush(data) {
        ...
        $('#pos-save').prop('disabled', true);
        $('#pos-save-mpesa').prop('disabled', true);
        $.ajax({
            method: 'POST',
            url: '/api/v1/stk/push',
            data: {
    ...
    function get_recent_mpesa_payments() {
        $.ajax({
            method: 'GET',
            url: '/sells/pos/get-recent-mpesa-payments',
            dataType: 'json',
            success: function (result) {
                let element = $('#mpesa-tbody');
                console.log('get_recent_mpesa_payments_result:', result);
                element.html('');
                let payments = result.data;

                let html = $('#mpesa-tbody');
    ...
    $(document).on('shown.bs.modal', '.mpesa_c2b_model', function () {
        $('.mpesa_c2b_model')
            .find('input')
            .filter(':visible:first')
            .focus()
    ...
    $(document).on('click', 'div.product_box', function () {
        ...
        let stk_push_on = $('#stkpush-on').val();
...
// <td>${payment.created_at}</td>
// <td><button class="btn btn-sm btn-info" id="use_transaction_btn" onclick="useTransaction(${payment.id})">Use</button></td>
// </tr>`;
...
function get_recent_mpesa_payments() {
    $.ajax({
        ...
        dataType: 'json',
        success: function (result) {
            let element = $('#recent_payments_table_body');
            element.html('');
            let payments = result.data;
            ...
            $('#mpesa-search').on('input', function () {
                let searchValue = $(this).val().toLowerCase();
                console.log(searchValue);

                let filteredPayments = payments.filter(payment => {
                    return (
                        payment.FirstName.toLowerCase().includes(searchValue) ||
                        payment.LastName.toLowerCase().includes(searchValue) ||
                        payment.MSISDN.toLowerCase().includes(searchValue) ||
                        payment.TransAmount.toLowerCase().includes(searchValue) ||
                        payment.TransID.toLowerCase().includes(searchValue)
...
// Function to update the table based on search results
...
function useTransaction(trans_id) {
    $.ajax({
        method: 'POST',
        url: `/sells/pos/use-transaction`,
        data: {
            trans_id: trans_id
        },
        dataType: 'json',
        success: function (result) {
            ...
            let input_container = $("#form_input_container");
            ...
            let amount_difference = total_cart_amount - data.TransAmount;
          ...
          <div class="form-group">
            <label for="transaction_amount">Cash Amount: </label>
            <input type="text" class="form-control" id="c2b_cashamount" placeholder="Enter Cash Amount">
           </div>`;
            }


            let form_input_data = `
            <h4>Transaction: <span class="text-success"> #${data.TransID}${data.FirstName}</span></h4>

            <div class="form-group">
           <label for="transaction_amount">Total Cart Amount: </label>
           <input type="text" class="form-control" id="total_cart_amount" value="${total_cart_amount}" readonly>
          </div>
            <div class="form-group">
            <label for="transaction_amount">Transaction Amount: </label>
            <input type="number" class="form-control" id="c2b_payment_amount" value="${data.TransAmount}" readonly>
           </div>
           <div class="form-group">
      
            <input type="hidden" class="form-control" id="c2b_payment_id" value="${data.id}" hidden>
            <input type="hidden" class="form-control" id="c2b_payment_phone" value="${data.MSISDN}" hidden>
           </div>
           `;
            input_container.append(form_input_data);
            input_container.append($cash_input);
            $("#recent_mpesa_payments_modal").modal('hide');
            $('#recent_payment_modal').modal('show');
        },
    });
...
$("#finalize_c2b_transaction").click(function () {
    let c2b_cashamount = $("#c2b_cashamount").val();
    let c2b_payment_amount = $("#c2b_payment_amount").val();
    let c2b_payment_id = $("#c2b_payment_id").val();
    let c2b_payment_phone = $("#c2b_payment_phone").val();
    var formDataObject = pos_form_obj.serializeArray().reduce(function (obj, item) {
        obj[item.name] = item.value;
        return obj;
    }, {});
    ...
    let unprocessed_cart_amount = formData.final_total;
    let total_cart_amount = parseFloat(unprocessed_cart_amount.replace(/[^\d.]/g, ''));
    let amount_difference = total_cart_amount - c2b_payment_amount;
    if (total_cart_amount <= 0) {
        toastr.error("No products are select on the POS");
        return false;
    }
    ...
    if (amount_difference > 0) {
        ...
        if (c2b_cashamount < amount_difference) {
            toastr.error(`Cash amount should be ${amount_difference}`);
            return false;
        }
    ...
    // console.log("form data: ", pos_form_mpesa.serialize())
    pos_form_mpesa.submit();
    markPaymentAsUsed(c2b_payment_id);
...
function markPaymentAsUsed(payment_id) {
    $.ajax({
        method: 'POST',
        url: `/sells/pos/payment_used`,
        data: {
...
function markSTKPushPaymentAsUsed(mpesa_trans_id) {
    $.ajax({
        method: 'POST',
        url: `/sells/pos/mpesa_stkpush_payment_used`,
        data: {
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1) {
    ...
    if (add_via_ajax) {
        ...
        var price_group = '';
        if ($('#price_group').length > 0) {
            //solve product out of stock error
            // console.log("Price group #price_group",$('#price_group').val())
            // price_group = parseInt($('#price_group').val());
            price_group = $('#price_group').val()
            console.log("#price_group: ", price_group);
        }
        ...
        if ($('#default_price_group').length > 0 &&
            ...
            console.log("#default_price_group: ", price_group);
        }
        ...
        if ($('#types_of_service_price_group').length > 0 &&
            ...
            console.log("#types_of_service_price_group: ", price_group);
        ...
        console.log("PRICE GROUP: ", price_group);
...
$(document).on('change', '#types_of_service_id', function () {
    ...
    console.log("Types of Service ID:", types_of_service_id)
    var location_id = $('#location_id').val();
...
$(document).on('show.bs.modal', '#recent_mpesa_payments_modal', function () {
    console.log('showing mpesa payments modal');
    get_recent_mpesa_payments()
    // console.log('showing mpesa payments modal');
});
