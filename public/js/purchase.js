$(document).ready(function () {
    ...
    $(document).on('change', '.inline_discounts', function () {
        ...
        console.log("purchase before discount: ", purchase_before_discount);
    ...
    $(document).on('change', '.profit_percent', function () {
        ...
        console.log("profit percentage: ", profit_percent);
...
$('input.purchase_unit_cost').change(function () {
    console.log("UNIT COST CHANGED");
...
function update_inline_profit_percentage(row) {
    ...
    // simon changed 2023
    if (profit_percent < 0) {
        $('input.profit_percent').css('border-color', 'red');
        $('input.profit_percent').css('color', 'red');
        toastr.error('Profit margin can not be negative');
        $('button#submit_purchase_form').attr('disabled', 'disabled');

    } else {
        $('input.profit_percent').css('border-color', '#d2d6de');
        $('input.profit_percent').css('color', '#555');
        $('button#submit_purchase_form').removeAttr('disabled');
    }
...
function checkProfitPercent() {
    var negativeFound = false;
    $('.profit_percent').each(function () {
...
$(document).on('click', 'button#submit_purchase_form', function (e) {
    ...
    console.log("SUBMITTING THE PUERCHAGE FORM");

    var profit_mergin_has_negative = checkProfitPercent();
    if (profit_mergin_has_negative) {
        toastr.error('Profit margin must not be negative');
        return false;
    }
